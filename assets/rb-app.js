...
(function () {
  ...
  $$('[data-google-reviews]').forEach((root) => {
    ...
    function avatarHtml(r, tone) {
      if (r.avatar) {
        return '<span class="grev-avatar" aria-hidden="true"><img src="' + r.avatar + '" alt="" width="48" height="48"></span>';
      }
    ...
    function itemHtml(r, idx) {
      const verified = r.verified
        ? '<span class="grev-verified" title="Verified review" aria-label="Verified"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="14" height="14" aria-hidden="true"><circle cx="8" cy="8" r="8" fill="currentColor"/><path fill="#fff" d="M6.7 10.85 4.4 8.55l.9-.9 1.4 1.4 3.6-3.6.9.9z"/></svg></span>'
        : '';
      let reply = '';
      if (r.reply) {
  ...
  /* Subtle immersive parallax — disabled on small screens and reduced motion */
  const stories = $$('[data-imm-story]');
  const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    ...
    const move = () => {
      ...
      if (window.innerWidth < 860) {
        stories.forEach((root) => {
          const bg = root.querySelector('[data-imm-layer="bg"] img');
          const fg = root.querySelector('[data-imm-layer="fg"]');
      ...
      stories.forEach((root) => {
        ...
        const bg = root.querySelector('[data-imm-layer="bg"] img');
        const fg = root.querySelector('[data-imm-layer="fg"]');
        if (bg) bg.style.transform = `translate3d(0, ${progress * 12}px, 0) scale(1.04)`;
        if (fg) fg.style.transform = `translate3d(0, ${progress * -14}px, 0) scale(${1 + Math.min(Math.abs(progress) * 0.03, 0.04)})`;
      });
    };
